...
## Weights
    ...
    for j in o1:
        ...
        GP = [int(x) if x.is_integer() else round(x,3) for x in GP]
        GP = [str(k) for k in GP]
        DS = a[1]/9 + 2*(a[2]/9)
        DS = int(DS) if DS.is_integer() else round(DS,3)
        ff = str(DS)+":"+",".join(GP[0:]) 
        # Cluster 1
        if j == '441':
...
wangyi_df.columns=wangyi.columns[:-1]
data=wangyi_df
...
with open(file_path+filename_out, "w") as f:
  for line in header:
      f.write(line + "\n")
  data_unify.to_csv(f,sep='\t',index=False,header=True)
